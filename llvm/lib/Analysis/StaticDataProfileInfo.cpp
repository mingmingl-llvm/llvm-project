...
void StaticDataProfileInfo::addGlobalProfileCount(
    const GlobalVariable *GV, std::optional<uint64_t> Count) {
  if (!Count)
    return;
  uint64_t &OriginalCount = GlobalProfileCounts[GV];
  OriginalCount += llvm::SaturatingAdd(*Count, OriginalCount);
  // Clamp the count to getInstrMaxCountValue. InstrFDO reserves a few
  // large values for special use.
  if (OriginalCount > getInstrMaxCountValue())
...
std::optional<uint64_t>
StaticDataProfileInfo::getGlobalProfileCount(const GlobalVariable *GV) const {
  auto I = GlobalProfileCounts.find(GV);
  if (I == GlobalProfileCounts.end())
    return std::nullopt;
  return I->second;
}
...
void StaticDataProfileInfoWrapperPass::print(raw_ostream &OS,
  ...
  for (const auto &I : Info->GlobalProfileCounts) {
    OS << "  " << I.first->getName() << ": " << I.second << "\n";
  }
}
